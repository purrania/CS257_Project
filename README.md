...
- For generating the data files from SAT Competition dataset, either add in the dimacs_tests.json file at our link, or by running the creation of a new selection of random clauses from a CNF (first make sure a .CNF file of your choosing is present in the same directory as test_case_maker.py) and then run test_case_maker.py. This will create a dimacs_tests.json file in the same directory, which contains the randomly selcted set of test clauses. 
